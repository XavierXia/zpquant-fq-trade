#include "FEMAS_FUTURE_MDPlugin.h"
#include <stdarg.h>
#include <thread>
#include "AutoPend.h"
const string CFEMAS_FUTURE_MDPlugin::s_strAccountKeyword = "serveraddress;brokerid;investorid;username;password;";
extern char ProcessName[256];
CFEMAS_FUTURE_MDPlugin::CFEMAS_FUTURE_MDPlugin():m_StartAndStopCtrlTimer(m_IOservice),m_abIsPending(false), m_adbIsPauseed(false)
{
}

CFEMAS_FUTURE_MDPlugin::~CFEMAS_FUTURE_MDPlugin()
{
}

bool CFEMAS_FUTURE_MDPlugin::IsPedding()
{
	return m_abIsPending.load();
}

bool CFEMAS_FUTURE_MDPlugin::IsOnline()
{
	return m_boolIsOnline;
}

void CFEMAS_FUTURE_MDPlugin::IncreaseRefCount()
{
	m_intRefCount++;
}

void CFEMAS_FUTURE_MDPlugin::DescreaseRefCount()
{
	m_intRefCount--;
}

int CFEMAS_FUTURE_MDPlugin::GetRefCount()
{
	return m_intRefCount;
}

void CFEMAS_FUTURE_MDPlugin::CheckSymbolValidity(const unordered_map<string, string> & insConfig)
{
...
string CFEMAS_FUTURE_MDPlugin::GetCurrentKeyword()
{
	return "femas_md&"+m_strUsername+"&"+m_strBrokerID;
}

string CFEMAS_FUTURE_MDPlugin::GetProspectiveKeyword(const ptree & in)
{
	string retKey = "femas_md&";
...
void CFEMAS_FUTURE_MDPlugin::GetState(ptree & out)
{
	if(m_boolIsOnline)
		out.put("online","true");
	else
...
void CFEMAS_FUTURE_MDPlugin::MDInit(const ptree & in)
{
	...
	if (temp != in.not_found())
	{
		...
		if (m_strInvestorID.size()>(sizeof(TUstpFtdcInvestorIDType) - 1))
			throw std::runtime_error("femase:invectorid is too long");
		else if (m_strInvestorID.empty())
			throw std::runtime_error("femase:invectorid is empty");
	}
	else
		throw std::runtime_error("femase:Can not find <invectorid>");
	...
	m_StartAndStopCtrlTimer.async_wait(boost::bind(
		&CFEMAS_FUTURE_MDPlugin::TimerHandler,
		this,
...
void CFEMAS_FUTURE_MDPlugin::MDHotUpdate(const ptree & NewConfig)
{
	MDUnload();
	m_IOservice.reset();
	MDInit(NewConfig);
...
void CFEMAS_FUTURE_MDPlugin::TimerHandler(boost::asio::deadline_timer* timer, const boost::system::error_code& err)
{
	if (err)
	{
...
bool CFEMAS_FUTURE_MDPlugin::Start()
{
	...
	if (false == m_boolIsOnline)
	{
		...
		m_pUserApi = std::shared_ptr<CUstpFtdcMduserApi>(CUstpFtdcMduserApi::CreateFtdcMduserApi(), [](CUstpFtdcMduserApi* p) {if (p) p->Release();});
		if (nullptr == m_pUserApi)
		{
			ShowMessage(
				severity_levels::error,
				"%s CreateFtdcMduserApi error",
		...
		m_pUserApi->SubscribeMarketDataTopic(100, USTP_TERT_QUICK);//飞马接口必须写这两条语句
		m_pUserApi->SubscribeMarketDataTopic(21001, USTP_TERT_QUICK);//否则收不到行情
...
void CFEMAS_FUTURE_MDPlugin::Stop()
{
	CAutoPend pend(m_abIsPending);
	if (true==m_boolIsOnline)
	{
		boost::shared_lock<boost::shared_mutex> lg(m_mapObserverStructProtector);//写锁
		...
		if (0 != m_pUserApi->UnSubMarketData(ppInstrumentID, InstrumentCount))
...
void CFEMAS_FUTURE_MDPlugin::MDUnload()
{
...
void CFEMAS_FUTURE_MDPlugin::Pause()
{
	m_adbIsPauseed.store(true);
}

void CFEMAS_FUTURE_MDPlugin::Continue()
{
	m_adbIsPauseed.store(false);
}

void CFEMAS_FUTURE_MDPlugin::ShowMessage(severity_levels lv, const char * fmt, ...)
{
	char buf[512];
	va_list arg;
	va_start(arg, fmt);
...
void CFEMAS_FUTURE_MDPlugin::MDAttachStrategy(
	MStrategy * strategy,
...
{
	boost::unique_lock<boost::shared_mutex> lg(m_mapObserverStructProtector);//写锁
	...
	else
	{
		...
		if (m_boolIsOnline)
		{
			...
			if (0 != m_pUserApi->SubMarketData(ppInstrumentID, 1))
				ShowMessage(
					severity_levels::error,
...
void CFEMAS_FUTURE_MDPlugin::MDDetachStrategy(MStrategy * strategy)
{
	boost::unique_lock<boost::shared_mutex> lg(m_mapObserverStructProtector);//写锁
	...
	for (auto & ins : m_mapStrategy2Insids[strategy])
	{
		...
		if (m_mapInsid2Strategys[ins].second.empty())
		{
			...
			if (m_boolIsOnline)
			{
				...
				if (0 != m_pUserApi->UnSubMarketData(ppInstrumentID, 1))
					ShowMessage(
						severity_levels::error,
...
void CFEMAS_FUTURE_MDPlugin::OnFrontConnected()
{
...
void CFEMAS_FUTURE_MDPlugin::OnFrontDisconnected(int nReason)
{
	ShowMessage(
		severity_levels::error, 
...
void CFEMAS_FUTURE_MDPlugin::OnHeartBeatWarning(int nTimeLapse)
{
	ShowMessage(
		severity_levels::error,
...
void CFEMAS_FUTURE_MDPlugin::OnPackageStart(int nTopicID, int nSequenceNo)
{
}

void CFEMAS_FUTURE_MDPlugin::OnPackageEnd(int nTopicID, int nSequenceNo)
{
}

void CFEMAS_FUTURE_MDPlugin::OnRspError(CUstpFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
...
void CFEMAS_FUTURE_MDPlugin::OnRspUserLogin(CUstpFtdcRspUserLoginField *pRspUserLogin, CUstpFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if ((pRspInfo) && (pRspInfo->ErrorID != 0))
	{
		m_boolIsOnline = false;
	...
	else
	{
		...
		if (0 != InstrumentCount)
		{
			...
			if (0 != m_pUserApi->SubMarketData(ppInstrumentID, InstrumentCount))
...
void CFEMAS_FUTURE_MDPlugin::OnRspUserLogout(CUstpFtdcRspUserLogoutField *pRspUserLogout, CUstpFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{}

void CFEMAS_FUTURE_MDPlugin::OnRspSubscribeTopic(CUstpFtdcDisseminationField * pDissemination, CUstpFtdcRspInfoField * pRspInfo, int nRequestID, bool bIsLast)
{

}

void CFEMAS_FUTURE_MDPlugin::OnRspQryTopic(CUstpFtdcDisseminationField * pDissemination, CUstpFtdcRspInfoField * pRspInfo, int nRequestID, bool bIsLast)
{
}

void CFEMAS_FUTURE_MDPlugin::OnRspSubMarketData(CUstpFtdcSpecificInstrumentField * pSpecificInstrument, CUstpFtdcRspInfoField * pRspInfo, int nRequestID, bool bIsLast)
{
	if (pRspInfo&&pRspInfo->ErrorID != 0)
	{
		ShowMessage(severity_levels::error,
...
void CFEMAS_FUTURE_MDPlugin::OnRspUnSubMarketData(CUstpFtdcSpecificInstrumentField * pSpecificInstrument, CUstpFtdcRspInfoField * pRspInfo, int nRequestID, bool bIsLast)
{
	if (pRspInfo&&pRspInfo->ErrorID != 0)
	{
		ShowMessage(severity_levels::error,
...
void CFEMAS_FUTURE_MDPlugin::OnRtnDepthMarketData(CUstpFtdcDepthMarketDataField *pDepthMarketData)
{
	if (m_adbIsPauseed)
		return;
	if (0 == pDepthMarketData->InstrumentID[0])
		return;
	boost::shared_lock<boost::shared_mutex> lg(m_mapObserverStructProtector);//读锁
