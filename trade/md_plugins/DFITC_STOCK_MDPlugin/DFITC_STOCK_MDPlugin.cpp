#include "DFITC_STOCK_MDPlugin.h"
#include <stdarg.h>
#include <thread>
#include "AutoPend.h"
#define NAME "dfitc_stock_md&"
const string CDFITC_STOCK_MDPlugin::s_strAccountKeyword= "serveraddress;accountid;password;";
extern char ProcessName[256];
CDFITC_STOCK_MDPlugin::CDFITC_STOCK_MDPlugin():m_StartAndStopCtrlTimer(m_IOservice), m_abIsPending(false), m_adbIsPauseed(false)
{
}

CDFITC_STOCK_MDPlugin::~CDFITC_STOCK_MDPlugin()
{
}

bool CDFITC_STOCK_MDPlugin::IsPedding()
{
	return m_abIsPending.load();
}

bool CDFITC_STOCK_MDPlugin::IsOnline()
{
	return m_boolIsOnline;
}

void CDFITC_STOCK_MDPlugin::IncreaseRefCount()
{
	m_intRefCount++;
}

void CDFITC_STOCK_MDPlugin::DescreaseRefCount()
{
	m_intRefCount--;
}

int CDFITC_STOCK_MDPlugin::GetRefCount()
{
	return m_intRefCount;
}

void CDFITC_STOCK_MDPlugin::CheckSymbolValidity(const unordered_map<string, string> & insConfig)
{
	...
	if (Type->second != "stock")
		throw std::runtime_error("dfitc_stock marketdatasource does not support this symbol.");
...
string CDFITC_STOCK_MDPlugin::GetCurrentKeyword()
{
	return NAME + m_strAccountID;
}

string CDFITC_STOCK_MDPlugin::GetProspectiveKeyword(const ptree & in)
{
	string retKey = NAME;
...
void CDFITC_STOCK_MDPlugin::GetState(ptree & out)
{
	if(m_boolIsOnline)
		out.put("online","true");
	else
...
void CDFITC_STOCK_MDPlugin::MDInit(const ptree & in)
{
	...
	m_StartAndStopCtrlTimer.async_wait(boost::bind(
		&CDFITC_STOCK_MDPlugin::TimerHandler,
		this,
...
void CDFITC_STOCK_MDPlugin::MDHotUpdate(const ptree & NewConfig)
{
	MDUnload();
	m_IOservice.reset();
	MDInit(NewConfig);
...
void CDFITC_STOCK_MDPlugin::TimerHandler(boost::asio::deadline_timer* timer, const boost::system::error_code& err)
{
	if (err)
	{
...
bool CDFITC_STOCK_MDPlugin::Start()
{
...
void CDFITC_STOCK_MDPlugin::Stop()
{
	CAutoPend pend(m_abIsPending);
	if (true==m_boolIsOnline)
	{
		...
		if (0 != m_pUserApi->UnSubscribeStockMarketData(ppInstrumentID, InstrumentCount, ++m_uRequestID))
		...
		int res = m_pUserApi->ReqStockUserLogout(&req);
		if (0 != res)
...
void CDFITC_STOCK_MDPlugin::MDUnload()
{
...
void CDFITC_STOCK_MDPlugin::Pause()
{
	m_adbIsPauseed.store(true);
}

void CDFITC_STOCK_MDPlugin::Continue()
{
	m_adbIsPauseed.store(false);
}

void CDFITC_STOCK_MDPlugin::ShowMessage(severity_levels lv, const char * fmt, ...)
{
	char buf[512];
	va_list arg;
	va_start(arg, fmt);
...
void CDFITC_STOCK_MDPlugin::MDAttachStrategy(
	MStrategy * strategy,
	...
	atomic_uint_least64_t * updatetime)
{
	...
	else
	{
		...
		if (m_boolIsOnline)
		{
			...
			if (0 != m_pUserApi->SubscribeStockMarketData(ppInstrumentID, 1,++m_uRequestID))
				ShowMessage(
					severity_levels::error,
...
void CDFITC_STOCK_MDPlugin::MDDetachStrategy(MStrategy * strategy)
{
	boost::unique_lock<boost::shared_mutex> lg(m_mapObserverStructProtector);//Ð´Ëø
	...
	for (auto & ins : m_mapStrategy2Insids[strategy])
	{
		...
		if (m_mapInsid2Strategys[ins].second.empty())
		{
			...
			if (m_boolIsOnline)
			{
				...
				if (0 != m_pUserApi->UnSubscribeStockMarketData(ppInstrumentID, 1, ++m_uRequestID))
					ShowMessage(
						severity_levels::error,
...
void CDFITC_STOCK_MDPlugin::OnFrontConnected()
{
	...
	if ((nullptr == m_pUserApi)||(0 != m_pUserApi->ReqStockUserLogin(&req)))
	{
		ShowMessage(
			severity_levels::error,
...
void CDFITC_STOCK_MDPlugin::OnFrontDisconnected(int nReason)
{
	ShowMessage(
		severity_levels::error, 
...
void CDFITC_STOCK_MDPlugin::OnRspStockUserLogin(struct DFITCSECRspUserLoginField * pRspUserLogin, struct DFITCSECRspInfoField * pRspInfo)
{
	...
	else
	{
		...
		if (0 != InstrumentCount)
		{
			...
			if (0 != m_pUserApi->SubscribeStockMarketData(ppInstrumentID, InstrumentCount, ++m_uRequestID))
...
void CDFITC_STOCK_MDPlugin::OnRspStockUserLogout(struct DFITCSECRspUserLogoutField * pRspUsrLogout, struct DFITCSECRspInfoField * pRspInfo)
{
	ShowMessage(
		severity_levels::normal,
...
void CDFITC_STOCK_MDPlugin::OnRspError(struct DFITCSECRspInfoField *pRspInfo)
{
...
void CDFITC_STOCK_MDPlugin::OnRspStockSubMarketData(struct DFITCSECSpecificInstrumentField * pSpecificInstrument, struct DFITCSECRspInfoField * pRspInfo)
{
	if (pRspInfo&&pRspInfo->errorID != 0)
	{
		ShowMessage(severity_levels::error,
...
void CDFITC_STOCK_MDPlugin::OnRspStockUnSubMarketData(struct DFITCSECSpecificInstrumentField * pSpecificInstrument, struct DFITCSECRspInfoField * pRspInfo)
{
	if (pRspInfo&&pRspInfo->errorID != 0)
	{
		ShowMessage(severity_levels::error,
...
void CDFITC_STOCK_MDPlugin::OnStockMarketData(struct DFITCStockDepthMarketDataField * pDepthMarketData)
{
	if (m_adbIsPauseed)
		return;
	...
	try {
		...
		tick.m_dbHighestPrice = pDepthMarketData->sharedDataField.highestPrice;
		tick.m_dbLowestPrice = pDepthMarketData->sharedDataField.lowestPrice;
		tick.m_dbPreClosePrice = pDepthMarketData->staticDataField.preClosePrice;

		tick.m_dbUpperLimitPrice = pDepthMarketData->staticDataField.upperLimitPrice;
		tick.m_dbLowerLimitPrice = pDepthMarketData->staticDataField.lowerLimitPrice;
